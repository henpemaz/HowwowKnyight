	...
	*/
    public class HowwowKnyight : Mod, ITogglableMod
    {
        ...
        public override void Initialize()
        {
            ...
            //ModHooks.Instance.LanguageGetHook += WanguageGet;
            ...
            /*if (ModHooks.LoadedMods.Contains("DebugMod")) {
                Log("Debug exists");
                try
                {
                    int Debugchooser = ReflectionHelper.GetField<DebugMod, int>("chooser");
                    Log($"Debug chooser is {Debugchooser}.");
                    if (Debugchooser == 1)
                    {
                        try
                        {
                            Log("DebugInter(); called");
                            DebugInter();
                        }
                        catch
                        {
                            Log("No debug present");
                        }
                    }
                    else if (UnityEngine.SceneManagement.SceneManager.GetActiveScene().name == "Menu_Title")
                    {
                        try
                        {
                            Log("Normal Titlechange");
                            ModifyTitweTextuweRuwtine = GameManager.instance.StartCoroutine(HowwowKnyight.ModifyTitweSpwite());
                        }
                        catch
                        {
                            Log("Unload in Initialize, caused by main texture");
                            Unload();
                        }
                    }
                    else
            ...
            if (ModHooks.LoadedMods.Contains("DebugMod"))
            {
                ...
                try
                {
                    int Debugchooser = ReflectionHelper.GetField<DebugMod.DebugMod, int>("chooser");
                    Log($"Debug chooser is {Debugchooser}.");
                    if (Debugchooser == 1)
                    {
                        bool DebughasEditedTitle = ReflectionHelper.GetField<DebugMod.DebugMod, bool>("hasEditedTitle");
                        Log($"Debug edited title is {DebughasEditedTitle}");

                        if (DebughasEditedTitle == false)
                        {
                            ReflectionHelper.SetField<DebugMod.DebugMod, bool>("shouldEditTitle", false);
                            //Start the couroutine for title
                            DebugInter();
                            ...
                            ModifyTitweTextuweRuwtine = GameManager.instance.StartCoroutine(HowwowKnyight.ModifyTitweSpwite());
                            */
                            Log("Sucess!! Debug's power has been contained");
                        }
                        else
                        {
                            //Start the couroutine for title
        ...
        {
            //How does this work wth
            //string text = orig(instance, key);
